...
uint8_t PS_GetImage(void)
{
  ...
  SendCheck(temp);
  data = JudgeStr(2000);
	HAL_UART_Transmit(&huart1,data,16,100);
...
uint8_t PS_SetAddr(uint32_t PS_addr)
{
  ...
  if(ensure == 0x00)
	...
	{
		HAL_UART_Transmit(&huart1,(uint8_t *)"设置地址失败！",16,100);
	}
  return ensure;
}
...
{
	HAL_UART_Transmit(&huart1,(uint8_t *)&x,16,100);
	HAL_UART_Transmit(&huart1,(uint8_t *)&y,16,100);
	HAL_UART_Transmit(&huart1,(uint8_t *)&ID,16,100);
...
void Add_FR(void)
{
  ...
  while(1)
  {
    ...
    case 0:
      ...
      if(ensure == 0x00)
      {
				HAL_UART_Transmit(&huart1,(uint8_t *)"获得图像成功",18,100);
        ensure = PS_GenChar(CharBuffer1); //生成特征
        if(ensure == 0x00)
        {
					HAL_UART_Transmit(&huart1,(uint8_t *)"    指纹正常    ",16,100);
      ...
      else HAL_UART_Transmit(&huart1,(uint8_t *)"获得图像失败",18,100);
      break;

    case 1:
      i++;
			HAL_UART_Transmit(&huart1,(uint8_t *)"   请再按一次   ",16,100);
    ...
    case 2:
			HAL_UART_Transmit(&huart1,(uint8_t *)"   对比两次指纹  ",16,100);
   
      ensure = PS_Match();
      if(ensure == 0x00)
      {
				HAL_UART_Transmit(&huart1,(uint8_t *)"    对比成功    ",16,100);
      
        processnum = 3; //跳到第四步
      }
      else
      {
				HAL_UART_Transmit(&huart1,(uint8_t *)"    对比失败    ",16,100);
    ...
    case 3:
			HAL_UART_Transmit(&huart1,(uint8_t *)"  生成指纹模板  ",16,100);

      HAL_Delay(500);
      ensure = PS_RegModel();
      if(ensure == 0x00)
      {
				HAL_UART_Transmit(&huart1,(uint8_t *)"生成指纹模板成功",16,100);
    ...
    case 4:
      ...
      if(ensure == 0x00)
      {
				HAL_UART_Transmit(&huart1,(uint8_t *)"  录入指纹成功  ",16,100);
    ...
    if(ensure == 0x00) //获取图像成功
    {
      ...
      if(ensure == 0x00) //生成特征成功
      {
        ...
        if(ensure == 0x00) //搜索成功
        {
         ...
         HAL_UART_Transmit(&huart1,(uint8_t *)str,18,100);
					
         
        }
        else
        {
					HAL_UART_Transmit(&huart1,(uint8_t *)"验证失败",10,100);
      	...
      	HAL_UART_Transmit(&huart1,(uint8_t *)"请按压手指",12,100);
    }
  }
...
void delete_ALL(void)
{
   ...
   if(ensure == 0)
   {
		 HAL_UART_Transmit(&huart1,(uint8_t *)" 清空指纹库成功 ",20,100);
