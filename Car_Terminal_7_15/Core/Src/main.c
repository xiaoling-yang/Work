...
unsigned char p[8]={0xEF,0XAA,0X12,0X00,0X00,0X00,0X00,0X12};
...
//					char p[10];
//					sprintf(p,"%d",distance);
//					HAL_UART_Transmit(&huart1,p,5,100);
		...
		if( step==1 && playSoundFlag==1)//当前为第一步，且打开了语音
		{
			...
			SYN_FrameInfo(0, (uint8_t *)"[v13][m1][t5]请进行面容识别");	
			}
			...
			if(step==1)//指纹模块触发
	{
       ...
       HAL_TIM_Base_Stop_IT(&htim2);//防止被定时器打断
		...
		HAL_UART_Transmit(&huart1,(uint8_t *)"面容识别通过",20,100);
		...
		else
		{
			...
			num=0;
			HAL_TIM_Base_Stop_IT(&htim2);
			HAL_UART_Transmit(&huart1,(uint8_t *)"面容识别失败",15,100);
			playSoundFlag=1;
		...
		else if(step==2)
		{
			...
			{
				playSoundFlag=0;
				SYN_FrameInfo(0, (uint8_t *)"[v7][m1][t5]面容识别通过，请进行酒精验证");
       ...
       HAL_TIM_Base_Stop_IT(&htim2);//防止被定时器打断
		...
		HAL_UART_Transmit(&huart1,(uint8_t *)str,len,100);	
			...
			else
			{
				...
				playSoundFlag=0;
				step=1;
				SYN_FrameInfo(0, (uint8_t *)"[v7][m1][t5]面容识别失败");
			}
