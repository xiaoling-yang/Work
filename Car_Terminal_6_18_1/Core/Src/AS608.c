...
uint8_t PS_SetAddr(uint32_t PS_addr)
{
  ...
  AS608Addr = PS_addr;
  if(ensure == 0x00)
		HAL_UART_Transmit(&huart1,"设置地址成功！",16,100);
    
  else
   HAL_UART_Transmit(&huart1,"设置地址错误！",16,100);
  return ensure;
}
...
uint8_t PS_ValidTempleteNum(uint16_t *ValidN)
{
  ...
  //  printf("\r\n有效指纹个数=%d", (data[10] << 8) + data[11]);
   ...
   HAL_UART_Transmit(&huart1,"读取指纹数错误！",16,100);
  return ensure;
}
...
void Add_FR(void)
{
  ...
  while(1)
  {
    ...
    case 0:
      i++;
		HAL_UART_Transmit(&huart1,"    请按手指    ",16,100);
      ...
      if(ensure == 0x00)
      {
        ...
        if(ensure == 0x00)
        {
					HAL_UART_Transmit(&huart1,"    指纹正常    ",16,100);
    ...
    case 1:
      i++;
		HAL_UART_Transmit(&huart1,"  请再按一次   ",16,100);
    ...
    case 2:
			HAL_UART_Transmit(&huart1,"  对比两次指纹  ",16,100);
    
      ensure = PS_Match();
      if(ensure == 0x00)
      {
				HAL_UART_Transmit(&huart1,"    对比成功    ",16,100);
        
        processnum = 3; //跳到第四步
      }
      else
      {
				HAL_UART_Transmit(&huart1,"    对比失败    ",16,100);
    ...
    case 3:
			HAL_UART_Transmit(&huart1,"  生成指纹模板  ",16,100);
   
      HAL_Delay(500);
      ensure = PS_RegModel();
      if(ensure == 0x00)
      {
				HAL_UART_Transmit(&huart1,"生成指纹模板成功",16,100);
    ...
    case 4:
      ...
      if(ensure == 0x00)
      {
				HAL_UART_Transmit(&huart1,"  录入指纹成功  ",16,100);
      ...
      {
        HAL_UART_Transmit(&huart1,"  录入指纹失败  ",16,100);
        processnum = 0;
        ShowErrMessage(ensure);
      }
      break;
...
void press_FR(void)
{
  ...
  while(1)
  {
    ...
    {
			HAL_UART_Transmit(&huart1,"获取图像成功",16,100);
      ensure = PS_GenChar(CharBuffer1);
      if(ensure == 0x00) //生成特征成功
      {
				HAL_UART_Transmit(&huart1,"生成特征成功",16,100);
        ensure = PS_HighSpeedSearch(CharBuffer1, 0, 99, &seach);
        if(ensure == 0x00) //搜索成功
        {
					HAL_UART_Transmit(&huart1,"指纹验证成功",16,100);
        ...
        {
					HAL_UART_Transmit(&huart1,"指纹验证失败",16,100);
...
void Del_FR(void)
{
  ...
  if(ensure == 0)
  {
		HAL_UART_Transmit(&huart1,"  删除指纹成功 ",16,100);
  
  }
  else
   HAL_UART_Transmit(&huart1,"  删除指纹失败 ",16,100);
  HAL_Delay(1500);
...
void delete_ALL(void)
{
   ...
   if(ensure == 0)
   {
		 HAL_UART_Transmit(&huart1," 清空指纹库成功",16,100);
   
   }
   else
	 {
		 HAL_UART_Transmit(&huart1,"清空指纹库失败 ",16,100);
