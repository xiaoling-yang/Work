...
//	AES128_DecryptData(&AES128_Handle, DataBuff, len);	//子操使:AES128解密128bit数据，len表示要解密数据的长度
  ...
  MX_ADC1_Init();
  MX_TIM2_Init();
  MX_USART1_UART_Init();//目前用于串口调试  PA9  PA10
  MX_USART2_UART_Init();//指纹模块AS608    PA2  PA3
  MX_USART3_UART_Init();//语音播报SYN6288 PB10  PB11
  MX_UART5_Init();//面容识别tx510   PC12  PD2
  MX_UART4_Init();//物联网模块
  MX_USART6_UART_Init();//距离传感器 PC6  PC7
  /* USER CODE BEGIN 2 */
